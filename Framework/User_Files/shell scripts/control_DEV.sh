...
clear
	function kill_gatling(){
		#
		# Function to kill all currently running remote Gatling jobs
		# Called from the main script below.
		#
		ssh -i "~/.ssh/tsummers-ec2.pem" root@ec2-54-155-136-115.eu-west-1.compute.amazonaws.com /usr/gatling-3.0.3/user-files/simulations/kill.sh
		echo "##########################"
		echo "Deleting these results files"
		echo "##########################"
		ssh -i "~/.ssh/tsummers-ec2.pem" root@ec2-54-155-136-115.eu-west-1.compute.amazonaws.com ls /usr/gatling-3.0.3/results/TEMP
		echo "##########################"
		echo "Deleting TEMP results files"
		echo "##########################"
		ssh -i "~/.ssh/tsummers-ec2.pem" root@ec2-54-155-136-115.eu-west-1.compute.amazonaws.com rm -rf /usr/gatling-3.0.3/results/TEMP/*
		ssh -i "~/.ssh/tsummers-ec2.pem" root@ec2-54-155-136-115.eu-west-1.compute.amazonaws.com ls /usr/gatling-3.0.3/results/TEMP
	...
	function start_gatling(){
		#
		# Funciton to start remote gatling jobs on the dedicated host.
		# Called from main script below.
		# 
		echo "=============================================================================================="
		echo 
		...
		case  $var  in
			[D,d])       
				...
				echo 
				echo "=============================================================================================="
				echo "Running with Default paraameters"
			...
			[O,o])
				...
				echo "Running..."
				#ssh -i "~/.ssh/tsummers-ec2.pem" root@ec2-54-155-136-115.eu-west-1.compute.amazonaws.com /usr/gatling-3.0.3/user-files/simulations/gat.sh $iterations $ramp $s1 $s2 $s3 $s4
	...
	echo "=============================================================================================="
	echo
	echo "Control remote Gatling jobs from your local machine."
	echo
	echo "=============================================================================================="
	echo
	echo "=============================================================================================="
	echo "=============================================================================================="
	read -p "Start a Gatling job or kill a running Gatling job. Enter S or K: " sk
		if [ "$sk" = "s" ];then
			start_gatling
		else
			kill_gatling
